...
from .models import (
    ActivityEvent, ApprovalRequest, Company, Contact, Deal, Interaction, Lead, PhaseRequirement, Project,
    RequirementTemplate, User, apply_cold_lead_status,
)
...
class TaskDueDateCalculationTests(APITestCase):
    def setUp(self):
        self.manager = User.objects.create_user(username='mgr', password='pass', role=User.Role.SALES_MANAGER)
        self.rep = User.objects.create_user(username='rep', password='pass', role=User.Role.SALES_REP)
        self.company = Company.objects.create(name='Acme', owner=self.rep)
        ...
        # Give two existing seed templates a duration before the lead (and
        # therefore its PhaseRequirements) exists, so generation picks it up.
        RequirementTemplate.objects.filter(label='Budget Proposal').update(default_duration_days=5)
        RequirementTemplate.objects.filter(label='Technical Specification').update(default_duration_days=10)

        self.lead = Lead.objects.create(company=self.company, contact=self.contact, assigned_to=self.rep)
        self.project = self.lead.project
        self.client.force_authenticate(self.manager)

    def test_phase_1_due_date_calculated_at_project_creation(self):
        requirement = self.project.requirements.get(label='Budget Proposal')
        self.assertIsNotNone(self.project.phase_1_started_at)
        self.assertEqual(requirement.due_date, self.project.phase_1_started_at.date() + timedelta(days=5))

    def test_task_with_no_duration_gets_no_due_date(self):
        requirement = self.project.requirements.get(label='Client Proposal Confirmation')
        self.assertIsNone(requirement.due_date)

    def test_phase_2_due_date_calculated_once_phase_2_starts(self):
        requirement = self.project.requirements.get(label='Technical Specification')
        self.assertIsNone(requirement.due_date)
        ...
        self.project.requirements.filter(phase=1).update(status=PhaseRequirement.Status.COMPLETED)
        approval = ApprovalRequest.objects.create(
            request_type=ApprovalRequest.RequestType.PHASE_1_SIGNOFF, project=self.project, requested_by=self.rep,
        )
        url = reverse('approvalrequest-detail', args=[approval.id])
        response = self.client.patch(url, {'status': ApprovalRequest.Status.APPROVED}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(requirement.due_date, self.project.phase_2_started_at.date() + timedelta(days=10))


class TaskDueDateComputedFieldsTests(ProjectRequirementsTestMixin, APITestCase):
    def test_effective_due_date_picks_the_earlier_date(self):
    ...
    def test_effective_due_date_falls_back_to_whichever_is_set(self):
        requirement = self.project.requirements.filter(phase=1).first()
        requirement.due_date = None
        requirement.committed_date = timezone.localdate() + timedelta(days=3)
        requirement.save()
        self.assertEqual(requirement.effective_due_date, requirement.committed_date)

    def test_is_overdue_true_when_due_date_passed_and_not_complete(self):
        requirement = self.project.requirements.filter(phase=1).first()
        requirement.due_date = timezone.localdate() - timedelta(days=1)
        requirement.save()
        self.assertTrue(requirement.is_overdue)

    def test_is_overdue_false_when_confirmed_complete(self):
        requirement = self.project.requirements.filter(phase=1).first()
        requirement.due_date = timezone.localdate() - timedelta(days=1)
    ...
    def test_completed_late_true_when_completed_after_due_date(self):
        requirement = self.project.requirements.filter(phase=1).first()
        requirement.due_date = timezone.localdate() - timedelta(days=5)
        requirement.status = PhaseRequirement.Status.COMPLETED
        requirement.save()
        self.assertTrue(requirement.completed_late)

    def test_completed_late_false_when_completed_on_time(self):
        requirement = self.project.requirements.filter(phase=1).first()
        requirement.due_date = timezone.localdate() + timedelta(days=5)
        requirement.status = PhaseRequirement.Status.COMPLETED
        requirement.save()
        self.assertFalse(requirement.completed_late)


class CommittedDateActivityEventTests(ProjectRequirementsTestMixin, APITestCase):
    def test_changing_committed_date_logs_administrative_event(self):
        requirement = self.project.requirements.filter(phase=1).first()
        ...
        self.assertIn('Committed date', event.description)
        self.assertIn(str(new_date), event.description)
        self.assertEqual(event.actor, self.manager)

    def test_setting_same_committed_date_does_not_log_an_event(self):
        requirement = self.project.requirements.filter(phase=1).first()
        requirement.committed_date = timezone.localdate() + timedelta(days=7)
        requirement.save()

        url = reverse('phaserequirement-detail', args=[requirement.id])
        response = self.client.patch(
            url, {'committed_date': requirement.committed_date.isoformat()}, format='json',
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertFalse(
    ...
    def test_rep_can_set_committed_date(self):
        requirement = self.project.requirements.filter(phase=1).first()
        self.client.force_authenticate(self.rep)
        new_date = timezone.localdate() + timedelta(days=7)
        url = reverse('phaserequirement-detail', args=[requirement.id])
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        requirement.refresh_from_db()
        self.assertEqual(requirement.committed_date, new_date)


class LeadStatusOverrideTests(ArchiveTestMixin, APITestCase):
    def test_manager_override_applies_immediately(self):
        self.client.force_authenticate(self.manager)
        url = reverse('lead-set-status', args=[self.lead.id])
        response = self.client.post(url, {'status': Lead.Status.HOT, 'reason': 'VIP client callback'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        self.lead.refresh_from_db()
        self.assertEqual(self.lead.status, Lead.Status.HOT)
        self.assertEqual(self.lead.status_override, Lead.Status.HOT)
        self.assertEqual(self.lead.status_override_reason, 'VIP client callback')
        self.assertEqual(self.lead.status_override_by, self.manager)
        self.assertIsNotNone(self.lead.status_override_at)
        self.assertTrue(
            ActivityEvent.objects.filter(
                lead=self.lead, category=ActivityEvent.Category.ADMINISTRATIVE,
                description__icontains='manually set',
            ).exists(),
        )

    def test_set_status_requires_a_reason(self):
        self.client.force_authenticate(self.manager)
        url = reverse('lead-set-status', args=[self.lead.id])
        response = self.client.post(url, {'status': Lead.Status.HOT}, format='json')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.lead.refresh_from_db()
        self.assertIsNone(self.lead.status_override)

    def test_rep_set_status_creates_pending_request(self):
        self.client.force_authenticate(self.rep)
        url = reverse('lead-set-status', args=[self.lead.id])
        response = self.client.post(url, {'status': Lead.Status.HOT, 'reason': 'Client called back'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        approval = ApprovalRequest.objects.get(
            lead=self.lead, request_type=ApprovalRequest.RequestType.STATUS_OVERRIDE,
        )
        self.assertEqual(approval.status, ApprovalRequest.Status.PENDING)
        self.assertEqual(approval.requested_status, Lead.Status.HOT)
        self.assertEqual(approval.reason, 'Client called back')
        self.assertEqual(approval.requested_by, self.rep)
        ...
        self.assertTrue(
            ActivityEvent.objects.filter(
                lead=self.lead, category=ActivityEvent.Category.ADMINISTRATIVE,
                description__icontains='requested',
            ).exists(),
        )

    def test_rep_cannot_set_status_on_a_lead_they_do_not_own(self):
        # LeadViewSet.get_queryset already scopes a SALES_REP to their own
        # assigned leads, so other_rep's request never finds this lead at
        # all -- 404, not 403 (same as any other lead they aren't on).
        self.client.force_authenticate(self.other_rep)
        url = reverse('lead-set-status', args=[self.lead.id])
        response = self.client.post(url, {'status': Lead.Status.HOT, 'reason': 'Nope'}, format='json')
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_rep_cannot_stack_duplicate_pending_requests(self):
        self.client.force_authenticate(self.rep)
        url = reverse('lead-set-status', args=[self.lead.id])
        first = self.client.post(url, {'status': Lead.Status.HOT, 'reason': 'First'}, format='json')
        self.assertEqual(first.status_code, status.HTTP_201_CREATED)
        second = self.client.post(url, {'status': Lead.Status.COLD, 'reason': 'Second'}, format='json')
        self.assertEqual(second.status_code, status.HTTP_400_BAD_REQUEST)

    def test_approving_status_override_request_applies_it(self):
        approval = ApprovalRequest.objects.create(
            request_type=ApprovalRequest.RequestType.STATUS_OVERRIDE,
            lead=self.lead, requested_by=self.rep, requested_status=Lead.Status.HOT, reason='Client called back',
        )
        self.client.force_authenticate(self.manager)
        url = reverse('approvalrequest-detail', args=[approval.id])
        response = self.client.patch(url, {'status': ApprovalRequest.Status.APPROVED}, format='json')
    ...
    def test_rejecting_status_override_request_leaves_lead_unchanged(self):
        ...
        url = reverse('approvalrequest-detail', args=[approval.id])
        response = self.client.patch(
            url, {'status': ApprovalRequest.Status.REJECTED, 'decision_note': 'Not convinced'}, format='json',
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_overridden_lead_not_changed_by_cold_lead_calculation(self):
        self.lead.status = Lead.Status.HOT
        self.lead.status_override = Lead.Status.HOT
        self.lead.status_override_reason = 'Pinned'
        self.lead.status_override_by = self.manager
        self.lead.status_override_at = timezone.now()
        self.lead.save()
        Lead.objects.filter(pk=self.lead.pk).update(last_activity_at=timezone.now() - timedelta(days=999))

        apply_cold_lead_status()

        self.lead.refresh_from_db()
        self.assertEqual(self.lead.status, Lead.Status.HOT)

    def test_non_overridden_stale_hot_lead_is_changed_by_cold_lead_calculation(self):
        self.lead.status = Lead.Status.HOT
        self.lead.save()
        Lead.objects.filter(pk=self.lead.pk).update(last_activity_at=timezone.now() - timedelta(days=999))
    ...
    def test_clear_status_override_restores_automatic_behaviour(self):
        self.lead.status = Lead.Status.HOT
        self.lead.status_override = Lead.Status.HOT
        self.lead.status_override_reason = 'Pinned'
        self.lead.status_override_by = self.manager
        ...
        self.client.force_authenticate(self.manager)
        url = reverse('lead-clear-status-override', args=[self.lead.id])
        response = self.client.post(url, {}, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)

        self.lead.refresh_from_db()
        self.assertIsNone(self.lead.status_override)
        self.assertEqual(self.lead.status_override_reason, '')
        self.assertIsNone(self.lead.status_override_by)
        self.assertTrue(
            ActivityEvent.objects.filter(
                lead=self.lead, category=ActivityEvent.Category.ADMINISTRATIVE,
                description__icontains='cleared',
            ).exists(),
        )

        # Automatic calculation now applies again.
        Lead.objects.filter(pk=self.lead.pk).update(last_activity_at=timezone.now() - timedelta(days=999))
        apply_cold_lead_status()
        self.lead.refresh_from_db()
        self.assertEqual(self.lead.status, Lead.Status.COLD)

    def test_rep_cannot_clear_status_override(self):
        self.lead.status_override = Lead.Status.HOT
        self.lead.save()
        self.client.force_authenticate(self.rep)
        url = reverse('lead-clear-status-override', args=[self.lead.id])
