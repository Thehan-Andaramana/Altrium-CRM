...
try {
    ...
    # -- 7. Done ----------------------------------------------------------------
    $frontendPort = Get-EnvValue (Join-Path $repoRoot 'frontend\.env') 'FRONTEND_PORT' '3000'
    $backendUrl = (Get-EnvValue (Join-Path $repoRoot 'frontend\.env') 'BACKEND_URL' 'http://localhost:9000').TrimEnd('/')
    ...
    Write-Host "  cd backend; .\.venv\Scripts\Activate.ps1; python manage.py runserver 9000"
    ...
    Write-Host "  ex1          Executive Manager"
